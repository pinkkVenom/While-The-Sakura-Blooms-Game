...
//ibject containing the image or video on a single graphic layer
...
public class GraphicObject
{
    ...
    //preventing duplicates of coroutines
    ...
    private void InitGraphic(bool immediate)
    {
        ...
        //set size of rect transform and anchors
    ...
    private Material GetTransitionMaterial()
    {
        ...
        if(mat != null)
        {
            //we have the material so we can return a clone
    ...
    private IEnumerator Fading(float target, float speed, Texture blend)
    {
        ...
        {
            float opacity = Mathf.MoveTowards(renderer.material.GetFloat(opacityParam), target, speed * Time.deltaTime * panelManager.DEFAULT_TRANSITION_SPEED);
            renderer.material.SetFloat(opacityParam, opacity);

            //make sure that video audio fades in at the same speed as opacity fading
            if (isVideo)
            {
        ...
        //destory image when it is faded out and no longer needed
        if(target == 0)
        {
            Destroy();
        }
